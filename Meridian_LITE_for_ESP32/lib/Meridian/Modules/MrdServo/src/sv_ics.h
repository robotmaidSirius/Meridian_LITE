...
float mrd_servo_process_ics(int a_servo_id, int a_cmd, float a_tgt, float a_tgt_past, float a_trim,
  ...
  if (a_cmd == 1) { // コマンドが1ならPos指定
    ...
    val_tmp = a_ics.setFree(a_servo_id);
  }

  // [DEBUG] L0 (ID=0) のサーボ通信結果を表示
  if (a_servo_id == 0) {
    Serial.print("[DBG_ICS] id=0 cmd=");
    Serial.print(a_cmd);
    Serial.print(" setPos_ret=");
    Serial.print(val_tmp);
  }
  ...
  float result_deg = a_mrd.Krs2Deg(val_tmp, a_trim, a_cw);

  // [DEBUG] L0 (ID=0) の変換結果を表示
  if (a_servo_id == 0) {
    Serial.print(" deg=");
    Serial.println(result_deg);
  }
...
void mrd_sv_drive_ics_double(Meridim90Union &a_meridim, ServoParam &a_sv,
  ...
  for (int i = 0; i < a_sv.num_max; i++) {
    ...
    // R系統サーボの処理
    if (a_sv.ixr_mount[i]) { // サーボがマウントされている場合
